...
Database:
      ...
      --db.enable-v2-static-files
          Explicitly enabled static file writers.

          For release / backwards compatibility purposes, some static file writers are disabled by default. See `read_only_segments` in `StaticFileProviderInner`.
